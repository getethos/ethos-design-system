...
// EDS core components
import { Footnote, Layout } from '../../index'
...
const DropdownChildren: React.SFC<DropdownChildrenProps> = ({
  ...
  // 3 || 4
  // 5 || 6
  const columns: [LinkItem[], LinkItem[]] = [[], []]
  const childItems = child.subnav.items
      ...
      <Layout.HorizontallyPaddedContainer>
        <div className={styles.childrenInner}>
          <div className={styles.childrenCta}>
            {child.subnav.cta?.href && (
              <NavLink
                href={child.subnav.cta?.href}
                LinkComponent={LinkComponent}
              >
                <DropdownCta
                  title={child.subnav.cta?.title || ''}
                  subcopy={child.subnav.cta?.subcopy || ''}
                />
              </NavLink>
            )}
          </div>
